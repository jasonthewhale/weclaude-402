/**
 * OAuth flow for Claude Code CLI tokens.
 *
 * Uses Anthropic's OAuth endpoint with PKCE to obtain access/refresh tokens.
 * These tokens authenticate requests as if they came from the real CLI.
 */

import type { PKCECodes, TokenData } from "./types.js";
...
export function generateAuthURL(state: string, pkce: PKCECodes): string {
  ...
  // Append scope with colons unencoded (matching real CLI behavior)
